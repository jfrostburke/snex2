...
def ingest_gw_galaxy_into_snex1(target_id, event_id, wrapped_session=None):
    ...
    o4_targets = _load_table('o4_targets', db_address=_snex1_address)

    existing_target = db_session.query(o4_targets).filter(o4_targets.target_id==target_id)
    if existing_target.count() > 0:
        if any([t.superevent_id == event_id for t in existing_target]):
            logger.info('Already ingested target {} into o4_targets table for event {}'.format(target_id, event_id))

        else:
            logger.info('Found existing target {} in o4_targets table, copying it'.format(target_id))
            existing_target_row = existing_target.first()
            existing_table = existing_target_row.__table__ #Somehow this is different than the o4_targets object
            ...
            db_session.add(o4_targets(**data))
        ...
        db_session.add(o4_targets(target_id=target_id, superevent_id=event_id, ra0=ra0, dec0=dec0))
    ...
    logger.info('Finished ingesting target {} into o4_targets'.format(target_id))
